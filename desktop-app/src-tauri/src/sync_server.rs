...
pub fn start_local_sync_server(state: Arc<AppState>) {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            let s = state.clone();
            let on_pairing = move |body: Vec<u8>| -> Vec<u8> {
                    ...
                    if !ciphertext_hex.is_empty() {
                        ...
                        if let Some(ref pair) = *kp_guard {
                            if let Ok(shared_secret) = core_crypto::decapsulate_pq_secret(
                                ...
                                {
                                    *s.session_key.lock().unwrap_or_else(|e| e.into_inner()) = sk;
                                    s.add_log(
                                        "[Session] Derived session key from KEM handshake"
                                            .to_string(),
                                    );
                                }
                                if !client_pk_hex.is_empty() {
                                    if let Ok(sas) = core_crypto::generate_sas_code(
                                        ...
                                        *s.sas_code.lock().unwrap_or_else(|e| e.into_inner()) =
                                            sas.clone();
                                        s.add_log(format!("[Session] SAS code: {sas}"));
                                    }
                                }
                                // Only transition to paired state if decapsulation succeeded
                                s.set_connection_status("ACTIVE".to_string());
                                s.set_connection_method("QUIC mTLS".to_string());
                                s.set_connection_color("green".to_string());
                                {
                                    let mut settings =
                                        s.settings.lock().unwrap_or_else(|e| e.into_inner());
                                    settings.is_paired = true;
                                    settings.paired_device_name = Some(name);
                                }
                                s.save_settings();
                                s.add_log("[Pairing] Paired via QUIC mTLS handshake".to_string());
                                return r#"{"status":"paired"}"#.to_string().into_bytes();
                            }
                        }
                    }
            ...
            let s = state.clone();
            let on_clipboard = move |body: Vec<u8>| -> Vec<u8> {
                let body_str = String::from_utf8_lossy(&body);
            ...
            let s = state.clone();
            let on_media = move |body: Vec<u8>| -> Vec<u8> {
                let body_str = String::from_utf8_lossy(&body);
            ...
            let on_poll = move || -> Vec<u8> {
                ...
                let resp = serde_json::json!({
                    ...
                    "sas_code": s.sas_code.lock().unwrap_or_else(|e| e.into_inner()).clone(),
