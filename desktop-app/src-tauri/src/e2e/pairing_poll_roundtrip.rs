//! Scenario: FULL pairing + poll + ratchet clipboard round-trip (audit P5-3).
//!
//! Extracted from the former `e2e.rs` monolith; the QUIC send/recv harness and
//! the hermetic server-identity setup live in `super`.

use super::{block_on_io_pub, hex_encode, reset_server_tls_identity, send_recv, send_recv_timeout};
