...
pub fn reconstruct_shamir_with_meta(
    ...
    for share in subset {
        if share.secret_hash != secret_hash || share.verify_key != verify_key {
            return Err(KyberError::CryptoError(
                "Share set is inconsistent — shares come from different splits or were substituted"
                    .into(),
            ));
        }
    ...
    let hash_ok: bool =
        subtle::ConstantTimeEq::ct_eq(recovered_hash.as_slice(), secret_hash.as_slice()).into();
    if !hash_ok {
        return Err(KyberError::CryptoError(
            "Reconstructed secret failed hash verification — a share was corrupted or substituted"
                .into(),
        ));
    }
    Ok(recovered)
...
mod tests {
    ...
    fn test_adaptive_forgery_caught_by_secret_hash() {
        ...
        assert!(
            verify_share_public(&forged),
            "public MAC is forgeable by design"
        );
